...
| `physicsnemo` | 2.0.0 | 已预装于 conda env | 无需额外处理，PyTorch ≥ 2.5 即可正常使用 |
...
### ~~问题 2：physicsnemo 启动时 DiT 模型导入失败~~ （已确认无需处理）
...
**根本原因：** DiT 依赖 `torch.distributed.tensor.placement_types`，该模块在 PyTorch 2.4 中不存在，**PyTorch 2.5 已包含**。

**正确解法：** 问题 1 中升级到 PyTorch ≥ 2.5 后，此错误自动消失，**无需对安装包做任何修改**。

> 注：曾误将此问题通过 patch `physicsnemo/models/__init__.py` 解决，后确认该 patch 是多余的，已撤销，安装包已恢复原始状态。
