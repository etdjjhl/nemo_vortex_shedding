...
# --- project monitoring (added) ---
from src.monitor.gpu_monitor import GPUMonitor
from src.monitor.tb_writer import TBWriter
...
def main(cfg: DictConfig) -> None:
    ...
    gpu_monitor = GPUMonitor(
        log_dir=cfg.gpu_log_dir, interval_sec=cfg.gpu_monitor_interval_sec
    )
    tb_writer = TBWriter(log_dir=cfg.tb_log_dir)

    trainer = MGNTrainer(cfg, rank_zero_logger)
    start = time.time()
    rank_zero_logger.info("Training started...")
    gpu_monitor.start(run_name="train")  # --- monitoring (added) ---
    ...
    for epoch in range(trainer.epoch_init, cfg.epochs):
        ...
        # --- monitoring (added) ---
        tb_writer.log_epoch(epoch, float(epoch_loss), lr=trainer.scheduler.get_last_lr()[0])
    ...
    gpu_monitor.stop()   # --- monitoring (added) ---
    tb_writer.close()    # --- monitoring (added) ---
